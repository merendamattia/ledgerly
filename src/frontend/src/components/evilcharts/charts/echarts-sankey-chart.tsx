...
export interface TooltipProps {
  ...
  valueFormatter?: (value: number) => string; // formats node and link values
}
...
function createTooltipFormatter(ctx: OptionBuildContext) {
  ...
  return (params: unknown): string => {
    ...
    if (p.dataType === "edge") {
      ...
      const value =
        typeof p.data?.value === "number"
          ? (tooltipSlot.valueFormatter?.(p.data.value) ?? p.data.value.toLocaleString())
          : "";
      return wrap(
        tooltipRow({
    ...
    const value = tooltipSlot.valueFormatter?.(nodeValue) ?? nodeValue.toLocaleString();
    return wrap(
      tooltipRow({
