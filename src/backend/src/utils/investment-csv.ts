...
export function parseInvestmentCsv(bytes: Uint8Array | ArrayBuffer): ParseResult {
  ...
  lines.forEach((rawLine, i) => {
    ...
    // Skip the header row.
    if (cells[0]?.toLowerCase().startsWith("ticket") || cells[0]?.toLowerCase().startsWith("ticker"))
      return;

    const raw = rawLine.trim();
    const [ticker, priceStr, qtyStr, totalStr, dateStr, broker] = cells;
    ...
    const parsedDate = parseDate(dateStr ?? "", "dd/MM/yyyy", new Date());
    ...
    const quantity = parseLocaleNumber(qtyStr ?? "");
    if (!Number.isFinite(quantity) || quantity <= 0) {
      errors.push({ line, raw, message: `Invalid quantity "${qtyStr ?? ""}"` });
      return;
    }
