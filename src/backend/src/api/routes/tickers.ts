...
export const tickersRoutes = new Hono<AppEnv>()
  .use("*", requireAuth)
  .get("/", async (c) => {
    const tickers = await tickerRepository.list(c.get("user").id);
  ...
  .post("/", zValidator("json", addAssetSchema), async (c) => {
    ...
    const ticker = await addAsset(c.get("user").id, symbol, type, isin);
    return c.json(ticker, 201);
  })
  .post("/manual", zValidator("json", addManualAssetSchema), async (c) => {
    const ticker = await addManualAsset(c.get("user").id, c.req.valid("json"));
    return c.json(ticker, 201);
  })
  .patch("/:id", zValidator("json", renameTickerSchema), async (c) => {
    const id = c.req.param("id");
    const { name } = c.req.valid("json");
    const ticker = await renameAsset(c.get("user").id, id, name);
    return c.json(ticker);
  })
  .post("/:id/price", zValidator("json", setManualPriceSchema), async (c) => {
    const id = c.req.param("id");
    const { price, date } = c.req.valid("json");
    await setManualPrice(c.get("user").id, id, price, date);
    return c.json({ ok: true });
  })
  .delete("/:id", async (c) => {
    const id = c.req.param("id");
    const existing = await tickerRepository.findById(c.get("user").id, id);
    if (!existing) throw new NotFoundError("Asset not found");
    await removeAsset(c.get("user").id, id);
    return c.json({ ok: true });
  });
